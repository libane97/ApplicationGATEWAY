import { Injectable } from '@angular/core';
import { HttpResponse } from '@angular/common/http';
import { Resolve, ActivatedRouteSnapshot, Routes, Router } from '@angular/router';
import { Observable, of, EMPTY } from 'rxjs';
import { flatMap } from 'rxjs/operators';

import { Authority } from 'app/shared/constants/authority.constants';
import { UserRouteAccessService } from 'app/core/auth/user-route-access-service';
import { ICheckup, Checkup } from 'app/shared/model/checkup.model';
import { CheckupService } from './checkup.service';
import { CheckupComponent } from './checkup.component';
import { CheckupDetailComponent } from './checkup-detail.component';
import { CheckupUpdateComponent } from './checkup-update.component';

@Injectable({ providedIn: 'root' })
export class CheckupResolve implements Resolve<ICheckup> {
  constructor(private service: CheckupService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<ICheckup> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((checkup: HttpResponse<Checkup>) => {
          if (checkup.body) {
            return of(checkup.body);
          } else {
            this.router.navigate(['404']);
            return EMPTY;
          }
        })
      );
    }
    return of(new Checkup());
  }
}

export const checkupRoute: Routes = [
  {
    path: '',
    component: CheckupComponent,
    data: {
      authorities: [Authority.USER],
      defaultSort: 'id,asc',
      pageTitle: 'djibGateApp.checkup.home.title',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: ':id/view',
    component: CheckupDetailComponent,
    resolve: {
      checkup: CheckupResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'djibGateApp.checkup.home.title',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: 'new',
    component: CheckupUpdateComponent,
    resolve: {
      checkup: CheckupResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'djibGateApp.checkup.home.title',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: ':id/edit',
    component: CheckupUpdateComponent,
    resolve: {
      checkup: CheckupResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'djibGateApp.checkup.home.title',
    },
    canActivate: [UserRouteAccessService],
  },
];
